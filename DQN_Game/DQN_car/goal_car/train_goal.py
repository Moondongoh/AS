...
class ObstacleAvoidanceEnv(gym.Env):
    ...
    #     turn_penalty = -2.0  # 회전 시 페널티 (조정 가능)
    #     straight_bonus = 3.0  # 직진 시 보너스 (조정 가능)
    #     action_reward = 0.0
    ...
    #     # 센서 기반 거리 보상 추가
    #     obs = self._get_observation()
    #     front_dist = obs[0]  # 0° 센서
    #     distance_reward = front_dist / self.sensor_max_range
    ...
    #     # 3) 목표 근처(near-goal) 보너스
    #     near_radius = 2.0  # 원하는 반경 설정 (셀 단위)
    #     if dist_to_goal < near_radius and not self.done:
    #         reward += 50.0  # near-goal bonus

    #     # 4) 정확히 목표점에 도달했을 때 최종 보상 및 종료
    #     goal_threshold = self.step_size * 0.1  # (≈0.1 셀 이내)
    #     if dist_to_goal <= goal_threshold:
    #         reward = 300.0  # goal bonus
    #         self.done = True

    #         # # 생존 보상 + 행동 보상 (직진 보너스 또는 회전 페널티)
    #         # reward = 1.5 + action_reward
    ...
    def step(self, action):
        ...
        # 6) 목표 도달 체크 (최우선)
        dist_to_goal = np.linalg.norm(self.agent_pos - self.goal_pos)
        goal_threshold = self.step_size * 0.1
        if dist_to_goal <= goal_threshold:
            self.done = True
        ...
        # 8) 목표 근처 보너스
        near_radius = 2.0
        if dist_to_goal < near_radius:
            reward += 5.0

        # 9) 행동 보상 합산
        reward += action_reward

        # 10) 스텝 수 체크 및 시간 초과 처리
        self.num_steps += 1
    ...
    def render(self, mode="human"):
        ...
        # 에이전트 방향 표시 (화살표)
        rad = math.radians(self.agent_heading)
        arrow_len = self.step_size * 1.5  # 화살표 길이
        self.ax.arrow(
            self.agent_pos[0],
            self.agent_pos[1],
...
def train_dqn(
    ...
    if save_model:
        torch.save(
            agent.policy_net.state_dict(), "dqn_obstacle_avoidance_weights_skrr.pth"
        )
        print(
            "\n모델 가중치를 dqn_obstacle_avoidance_weights_skrr.pth 에 저장하였습니다."
        )
...
if __name__ == "__main__":
    ...
    agent, rewards = train_dqn(num_episodes=4000, max_steps=200, render_interval=50)
