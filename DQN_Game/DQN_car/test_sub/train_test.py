...
class ObstacleAvoidanceEnv(gym.Env):
    ...
    def __init__(
        ...
        # 목표 지점 설정 (x=map_width-0.1, y=map_height/2)
...
def train_dqn(
    ...
    if save_model:
        torch.save(
            agent.policy_net.state_dict(), "dqn_obstacle_avoidance_weights_test.pth"
        )
        print(
            "\n모델 가중치를 dqn_obstacle_avoidance_weights_test.pth 에 저장하였습니다."
        )
...
if __name__ == "__main__":
    ...
    agent, rewards = train_dqn(num_episodes=3500, max_steps=100, render_interval=50)
